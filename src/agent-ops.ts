...
export function createAgentFile(options: CreateAgentOptions): AgentConfig {
	const dir =
		options.scope === "project"
			? findOrCreateProjectAgentsDir(options.cwd)
			: findOrCreateUserAgentsDir();
...
function findOrCreateUserAgentsDir(): string {
	const dir = getUserAgentsDir();
	fs.mkdirSync(dir, { recursive: true });
	return dir;
}

function findOrCreateProjectAgentsDir(cwd: string): string {
	// Try to find existing project agents dir
	const existing = findProjectAgentsDir(cwd);
	if (existing) return existing;

	// Create .pi/agents in current cwd
...
/**
 * Normalize an agent name: lowercase, replace spaces/special chars with hyphens
 */
